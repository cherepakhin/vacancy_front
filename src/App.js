...
function mapStateToProps(state) { // перенос из state в this.props....
    console.log("App. mapStateToProps:"+JSON.stringify(state.vacancies)); // App. mapStateToProps:{"vacancies":[{"completed":false,"id":-1,"title":"не показывать -1"}]}
    console.log("App. state.vacancies:"+JSON.stringify(state.vacancies)); // App. state.vacancies:[{"completed":false,"id":-1,"title":"не показывать -1"}]
    return {
        vacancies: state.vacancies // "vacanciesFromState:" внутри компонета доступно через this.props.vacancies
    }
}
